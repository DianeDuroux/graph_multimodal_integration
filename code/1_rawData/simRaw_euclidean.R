...
#Euclidean
distall=distRaw(exp)
Kall=affinityMatrix(distall)
fwrite(Kall, "Kgene_euclidean.txt")
...
distall=distRaw(exp)
Kall=affinityMatrix(distall)
fwrite(Kall, "Kimage_euclidean.txt")
...
distall=distRaw(exp)
Kall=affinityMatrix(distall)
fwrite(Kall, "Kearly_euclidean.txt")

#######
# SNF #
#######
KTrain_gene=as.matrix(fread("Kgene_euclidean.txt"))
KTrain_images=as.matrix(fread("Kimage_euclidean.txt"))
Ktrain=SNF(list(KTrain_gene, KTrain_images), K=5, t=5)
fwrite(Ktrain, "K_SNF_euclidean.txt")
...
# Average #
###########
K_GeneExpression=as.matrix(fread("Kgene_euclidean.txt"))
...
K_images=as.matrix(fread("Kimage_euclidean.txt"))
...
fwrite(KTrain, "KTrain_average_euclidean.txt")
fwrite(KTest, "KTest_average_euclidean.txt")
