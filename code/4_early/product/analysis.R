...
library(dplyr)
library(stringr)
setwd("/home/durouxd/April/brainCancer/4_Early/produit")
...
# Options #
###########
nsel=600 #Number of most variable features selected
cor_edges=0.25
...
genesExpr=fread("variablesGenes600.txt")
ImageFeatures=fread("variablesImages300.txt")
genesToKeep=c(genesExpr$genesToKeep, ImageFeatures$genesToKeep)
nsel=length(genesToKeep)
fwrite(data.frame(genesToKeep), paste("variables", nsel, ".txt", sep=""))
dat <- as.matrix(exp[genesToKeep, ])
