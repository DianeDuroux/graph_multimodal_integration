...
library(stringr)
library(SNFtool)
setwd("/home/durouxd/April/brainCancer/5_intermediate/lioness")
...
#Gene expression
KTrain_GeneExpression=as.matrix(fread(paste("simMatrix_", nsel_gene, "topGenes_", cor_edges_gene, "corEdges_", meth, "_scaled", scaled, "_train.txt", sep="")))
KTest_GeneExpression=as.matrix(fread(paste("simMatrix_", nsel_gene, "topGenes_", cor_edges_gene, "corEdges_", meth, "_scaled", scaled, "_test.txt", sep="")))
KTest_GeneExpression=as.matrix(KTest_GeneExpression[-1,])

#Images
KTrain_images=as.matrix(fread(paste("simMatrix_", nsel_image, "topGenes_", cor_edges_image, "corEdges_", meth, "_scaled", scaled, "_train.txt", sep="")))
KTest_images=as.matrix(fread(paste("simMatrix_", nsel_image, "topGenes_", cor_edges_image, "corEdges_", meth, "_scaled", scaled, "_test.txt", sep="")))
KTest_images=as.matrix(KTest_images[-1,])

#SNF
#Ktrain_SNF=SNF(list(KTrain_GeneExpression, KTrain_images), K=20, t=20)
#Ktest_SNF=SNF(list(KTest_GeneExpression, KTest_images), K=20, t=20)
...
rownames(results)=c("nbNodes",  "macroF1train", "corresponding_macroF1test", "Cparam_SVM")
  ...
  write.csv(results, paste("results_", nsel, "topGenes_", meth, "_scaled", scaled, ".txt", sep=""), row.names=T)
